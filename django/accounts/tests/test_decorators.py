...
def test_require_groups_allows_user_with_matching_group(rf, db):
    ...
    request.user = user
    assert require_groups(['pl:viewer'])(_ok_view)(request).status_code == 200
...
def test_require_groups_redirects_unauthenticated(rf, db):
    ...
    response = require_groups(['pl:editor'])(_ok_view)(request)
    assert response.status_code == 302
...
def test_require_groups_or_logic_allows_any(rf, db):
    ...
    request = rf.get('/editor/')
    request.user = user
    assert require_groups(['pl:editor', 'pl:admin'])(_ok_view)(request).status_code == 200
